...
void			test_ft_strsub_big_start(void *ptr) {
	char	*(*ft_strsub)(const char *, size_t, size_t) = ptr;
	SET_EXPLANATION("your strsub does not work when start >= ft_strlen(s)");

	SANDBOX_RAISE(
			char	*str = "01234";
			size_t	size = 10;

			char	*ret = ft_strsub(str, 10, size);
			if (!strncmp(ret, "", 1)) {
				free(ret);
				exit(TEST_SUCCESS);
			}
...
void            test_ft_strsub(void){
	...
	add_fun_subtest(test_ft_strsub_big_start);
}
...
void			test_ft_strsplit_terminator(void *ptr) {
	char	**(*ft_strsplit)(char *, char) = ptr;
	SET_EXPLANATION("your split will segfault in case --> *str=\"\\0aa\\0bbb\" c=\'\\0\' ");

	SANDBOX_RAISE(
			char	**expected = ft_strsplit("\0aa\0bbb", '\0');
...
void            test_ft_strsplit(void) {
	...
	add_fun_subtest(test_ft_strsplit_terminator);
